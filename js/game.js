...
class BrickGame {
    ...
    handleMouseMove(e) {
        ...
        const scale = this.canvas.width / rect.width; // 添加缩放比例
        const scaledX = mouseX * scale; // 根据缩放比例调整坐标
        
        // 只有在游戏中才移动挡板
        if (this.currentState === this.states.PLAYING) {
            this.player.x = scaledX - this.player.width / 2;
            ...
            if (this.debugMode) {
                console.log(`鼠标位置: ${scaledX}, 挡板位置: ${this.player.x}`);
            }
        }
    }
    ...
    handleTouch(e) {
        ...
        if (e.touches && e.touches.length > 0) {
            ...
            const scale = this.canvas.width / rect.width; // 添加缩放比例
            const scaledX = touchX * scale; // 根据缩放比例调整坐标
            
            if (this.debugMode) {
                console.log(`触摸事件 - 位置: ${scaledX}, 类型: ${e.type}`);
            }
